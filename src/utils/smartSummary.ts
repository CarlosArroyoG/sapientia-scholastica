...
const scoreSentence = (sentence: string, freq: Map<string, number>): number => {
  ...
  const lengthFactor = Math.min(tokens.length, 32) / 32;
  return score * (0.6 + lengthFactor);
};
...
const selectTopSentences = (sentences: string[], amount: number): string[] => {
  ...
  const unique = Array.from(new Map(scored.map((item) => [item.sentence.toLowerCase(), item])).values())
...
const chunkParagraphs = (text: string, targetSize = 650): string[] => {
  ...
  sentences.forEach((sentence) => {
    if ((current + ' ' + sentence).trim().length > targetSize && current.length > 0) {
...
export const buildSmartDocument = (pageTexts: string[], fileName: string): SmartDocument => {
  const cleanedPages = pageTexts.map(cleanText).filter((page) => page.length > 0);
  ...
  const overview = highlights.length > 0
    ? highlights.join(' ')
  ...
  const sections = cleanedPages.slice(0, 8).map((page, index) => {
    ...
    return {
      ...
      text: pageSummary.join(' ') || page.slice(0, 280)
    };
  });
